...
CREATE TABLE Grupa(
	...
	terenID int not null,
	trening datetime not null
);


CREATE TABLE Pripada_Grupi(
	grupaID int not null,
	igracID int not null
);

ALTER TABLE Trener ADD CONSTRAINT PK_Trener PRIMARY KEY (trenerID);
ALTER TABLE Grupa ADD CONSTRAINT PK_Grupa PRIMARY KEY (grupaID);
ALTER TABLE Pripada_Grupi ADD CONSTRAINT PK_Pripada_Grupi PRIMARY KEY (grupaID,igracID);

ALTER TABLE Grupa ADD CONSTRAINT FK_Grupa_trenerID FOREIGN KEY (trenerID) REFERENCES Trener(trenerID);
ALTER TABLE Grupa ADD CONSTRAINT FK_Grupa_terenID FOREIGN KEY (terenID) REFERENCES Teren(terenID);
ALTER TABLE Pripada_Grupi ADD CONSTRAINT FK_Pripada_Grupi_grupaID FOREIGN KEY (grupaID) REFERENCES Grupa(grupaID);
ALTER TABLE Pripada_Grupi ADD CONSTRAINT FK_Pripada_Grupi_igracID FOREIGN KEY (igracID) REFERENCES Igrac(igracID);

CREATE OR ALTER TRIGGER nedelja
ON Grupa
INSTEAD OF INSERT 
AS
BEGIN
	IF((SELECT COUNT(G.grupaID) FROM Grupa AS G, inserted AS I WHERE G.grupaID = I.grupaID AND DATEPART(WEEK,G. trening) = DATEPART(WEEK,I.trening) AND MONTH(G.trening) = MONTH (I.trening)) = 0 )
		INSERT INTO Grupa (grupaID,trenerID,naziv,terenID,trening)
			(SELECT grupaID,trenerID,naziv,terenID,trening FROM inserted)
	ELSE
		PRINT 'Trening se odrzava jednom nedeljno!'
END;
